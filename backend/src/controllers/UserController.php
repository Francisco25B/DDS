...
class UserController {
    ...
    public function users(Request $request, Response $response): Response {
        ...
        $page = $request->getQueryParams()['page'] ?? 1; // Página actual (por defecto la 1)
        $perPage = $request->getQueryParams()['perPage'] ?? 10; // Cantidad de registros por página (por defecto 10)
    
        try {
            $offset = ($page - 1) * $perPage; // Calcular el desplazamiento (offset)
            ...
            // Agregar la paginación (LIMIT y OFFSET)
            ...
            $stmt->bindValue(':perPage', $perPage, PDO::PARAM_INT); // Limitar los resultados por página
            $stmt->bindValue(':offset', $offset, PDO::PARAM_INT); // Desplazamiento para la paginación
    
            $stmt->execute();
            $users = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
            // Para obtener el total de usuarios que cumplen con los filtros (sin paginación)
            $countSql = "SELECT COUNT(*) FROM usuario WHERE 1=1";
            if (!empty($searchTerm)) {
                $countSql .= " AND (nombre LIKE :searchTerm OR apellido LIKE :searchTerm OR email LIKE :searchTerm OR direccion LIKE :searchTerm OR telefono LIKE :searchTerm)";
            }
            if (!empty($tipoUsuario)) {
                $countSql .= " AND tipo_usuario_id = :tipoUsuario";
            }
            if ($status !== '') {
            ...
            if (!empty($tipoUsuario)) {
                $countStmt->bindValue(':tipoUsuario', $tipoUsuario);
            }
            if ($status !== '') {
            ...
            $countStmt->execute();
            $totalUsers = $countStmt->fetchColumn(); // Total de usuarios que cumplen con los filtros
    
            return $this->jsonResponse($response, [
                'status' => 'success',
                'data' => $users,
                'total' => $totalUsers, // Devolver el total de usuarios
                'page' => $page, // Número de la página actual
                'perPage' => $perPage, // Número de resultados por página
            ], 200);
        } catch (\PDOException $e) {
